<?php
/**
 * The template file for creating custom menu
 *
 * @package WordPress
...
global $sid;
$sid = 0;
class delve_nav_walker extends Walker_Nav_Menu {
...
//	function start_lvl( &$output, $depth ) {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if( is_page_template('page-templates/single-page.php') ) {
			...
			//$attributes .= ! empty( $object->url ) ? ' href="'.get_bloginfo('url').'/#' . $varpost->post_name .'"' : '';
			//print_r($item);
			
			
			if($item->object == 'page') {
				$attributes .= ! empty( $item->url ) ? ' href="#'.$item->object_id.'" class="page-scroll"' : '';
    		}
